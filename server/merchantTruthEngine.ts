...
export function evaluateMerchantTruth(m: any): MerchantTruthResult {
  ...
  // All four "evidence-complete" signals firing together — contact route,
  // two independent proofs, strong proof, and a real buying signal — with
  // no rejection flags is itself enough to approve, even when the raw score
  // is in the 65–72 borderline band. Without this rung, a fully-evidenced
  // merchant gets demoted to REVIEW and silently dropped by
  // filterSalesReady's default `includeReview: false`.
  const evidenceComplete =
    hasContact &&
    hasTwoIndependentProofs &&
    hasStrongProof &&
  ...
  if (rejectionReasons.length && scores.finalSalesScore < 55) {
    ...
    decision = 'APPROVE';
  } else if (evidenceComplete && scores.finalSalesScore >= 65) {
    decision = 'APPROVE';
